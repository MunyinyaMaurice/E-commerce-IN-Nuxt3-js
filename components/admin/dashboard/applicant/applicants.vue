<template>
    ...
    <div>
      ...
      <h1 class="text-2xl text-center text-decoration-line: underline font-bold mb-4">Applicants</h1>
        ...
        <table class="w-full border-collapse">
          <thead>
            <tr class="bg-gray-200">
              ...
              <th class="py-2 px-4 border">Degree ID</th>
          ...
          <tbody>
            ...
            >
              <td class="py-2 px-4 border">{{ applicant.userId }}</td>
              ...
              <td class="py-2 px-4 border">{{ applicant.degreeId }}</td>
              <td class="py-2 px-4 border">{{ applicant.majorFocus }}</td>
              <td class="py-2 px-4 border">{{ applicant.role }}</td>
              
              <td class="py-2 px-4 border">
                <div class="relative inline-block">
                  <button
                    @click="toggleDropdown(applicant.userId)"
                  ...
                  <div
                    v-show="dropdownVisible(applicant.userId)"
                    class="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none"
                  >
                    <button
                      @click="updateToVolunteer(applicant.userId)"
                      class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100"
                    >
                      Update
                    </button>
                    <button
                      @click="cancelVolunteer(applicant.userId)"
                      class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100"
                    >
                      Delete
                    </button>
  ...
  const fetchApplicants = async () => {
    ...
    try {
      const {
        ...
        error: fetchError,
        refresh,
      } = await useIFetch<Applicant[]>("applicants", { lazy: false });
