<template>
  <div class="container mx-auto p-4">
    ...
    <h1 class="text-2xl text-center font-bold mb-4 underline">TASK LIST</h1>
...
// Define functions to handle dropdown and navigation
...
const deleteTask = async (taskId: number) => {
  try {
    ...
    if (error.value) {
      ...
      // Optionally, refetch the task list after a successful deletion
      await fetchTasks();
    }
  } catch (err) {
...
// Use the user store to fetch and manage user data
...
onMounted(async () => {
  ...
  //     } else if (role === 'ADMIN') {
  //       navigateTo('/dashboard/department');
  //     } else if (role === 'COORDINATOR') {
  //       navigateTo('/dashboard/task');
  //     } else if (role === 'VOLUNTEER') {
  //       navigateTo('/dashboard/assignedTask');
  //     }
  //   } catch (error) {
