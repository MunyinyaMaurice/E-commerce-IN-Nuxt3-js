    ...
    <div class="flex justify-between mb-4">
      <div>
        <button
          v-if="refreshData"
          @click="refreshData"
          class=" text-white font-bold py-2 px-4 rounded h-10 md:h-10 lg:h-10"
        >
          <!-- REFRESH -->
        </button>
      </div>
...
const fetchDepartments = async () => {
  ...
  try {
    const { data, error: fetchError, refresh } = await useIFetch('departments');
    if (fetchError.value) {
...
const navigateToCreateDepartment = () => {
  router.push('/dashboard/department/createDepartment');
};
...
const openUpdateForm = (departmentId: number) => {
  router.push(`/dashboard/department/updateDepartment/${departmentId}`);
};
